...
export const generateOrderId = (): string => {
  try {
    ...
    console.log('订单号长度:', orderId.length);
    console.log('订单号类型:', typeof orderId);
    console.log('=== generateOrderId 结束 ===');
    
    // 验证订单号
    if (!orderId || orderId.length < 10) {
      throw new Error('生成的订单号无效');
    }
    
    return orderId;
  } catch (error) {
    console.error('生成订单号时出错:', error);
    // 返回一个备用订单号
    const fallbackId = `MF${Date.now()}${Math.floor(Math.random() * 10000)}`;
    console.log('使用备用订单号:', fallbackId);
    return fallbackId;
  }
};
...
export const generateSign = (params: Record<string, string>, key: string): string => {
  ...
  sortedKeys.forEach(paramKey => {
    signStr += `${paramKey}=${filteredParams[paramKey]}&`;
  });
...
export const buildPaymentUrl = (
  ...
  try {
    console.log('=== buildPaymentUrl 被调用 ===');
    ...
    console.log('=== buildPaymentUrl 结束 ===');
    
    return finalUrl;
  } catch (error) {
    console.error('构建支付URL时出错:', error);
    throw error;
  }
};
...
export const debugPaymentParams = (
  ...
  return {
    ...
    signString: signStr,
    sign: sign,
    fullUrl: `${BASE_CONFIG.API_URL}submit.php?${params.toString()}`
  };
}; 